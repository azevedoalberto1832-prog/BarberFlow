...
function applyTenantBrand(shop){
  const custom=shop?.theme==="custom";
  document.body.classList.toggle("tenant-custom",custom);
  if(!custom) return;
  const primary=shop.primary_color||"#6d5dfb";
  const secondary=shop.secondary_color||"#22c3a6";
...
async function loadPublicData() {
  try {
    ...
    db.settings = { shop:shop.name, address:shop.address || "Endereço a confirmar", phone:shop.phone || "", open:shop.opening_time?.slice(0,5) || "09:00", close:shop.closing_time?.slice(0,5) || "19:00", breakStart:shop.break_start?.slice(0,5) || "", breakEnd:shop.break_end?.slice(0,5) || "", greeting:shop.whatsapp_message || "Olá! Agende seu horário pela Chrona.", instagram:shop.instagram || "", logo:shop.logo_url || "", description:shop.public_description || "Escolha o serviço, o profissional e o melhor horário para você.", primaryColor:shop.primary_color || "#6d5dfb", secondaryColor:shop.secondary_color || "#22c3a6" };
...
function openTenantForm(){
  ...
  document.body.insertAdjacentHTML("beforeend",`<div class="modal onboarding-modal" id="tenant-modal"><div class="modal-card" style="max-width:820px"><div class="modal-head"><div><div class="eyebrow">ONBOARDING CHRONA</div><h3>Nova empresa</h3><p class="muted">Cadastre a identidade e já libere o primeiro acesso.</p></div><button class="btn btn-ghost" data-tenant-close>✕</button></div><form id="tenant-form" class="modal-body"><div class="onboarding-section"><div class="section-number">01</div><div><h4>Informações da empresa</h4><p>Dados que aparecem na página pública.</p></div></div><div class="form-grid"><label class="field full"><span>Nome da empresa *</span><input name="name" placeholder="Ex.: Raquel Beauty" required></label><label class="field"><span>Slug do link *</span><input name="slug" pattern="[a-z0-9]+(?:-[a-z0-9]+)*" placeholder="raquel-beauty" required><small>Endereço curto, sem espaços ou acentos.</small></label><label class="field"><span>Segmento *</span><select name="business"><option value="beauty">Beleza e estética</option><option value="barber">Barbearia</option><option value="lash">Lash Designer</option><option value="wellness">Bem-estar</option><option value="health">Saúde</option><option value="services">Outros serviços</option></select></label><label class="field"><span>WhatsApp *</span><input name="phone" inputmode="tel" placeholder="(34) 99999-9999" required></label><label class="field"><span>Instagram</span><input name="instagram" placeholder="@empresa"></label><label class="field full"><span>Endereço</span><input name="address" placeholder="Rua, número, bairro e cidade"></label><label class="field full"><span>Descrição pública</span><textarea name="description" rows="3" placeholder="Conte em uma frase o que torna este atendimento especial."></textarea></label><label class="field full"><span>Link da logo</span><input name="logoUrl" type="url" placeholder="https://..."><small>Opcional. Pode ser configurado depois.</small></label></div><div class="onboarding-section"><div class="section-number">02</div><div><h4>Identidade visual</h4><p>As cores serão aplicadas automaticamente à agenda.</p></div></div><div class="brand-fields"><label class="color-field"><input name="primaryColor" type="color" value="#6d5dfb"><span><b>Cor principal</b><small>Botões e destaques</small></span></label><label class="color-field"><input name="secondaryColor" type="color" value="#22c3a6"><span><b>Cor secundária</b><small>Detalhes e contraste</small></span></label><label class="field"><span>Plano</span><select name="plan"><option value="essential">Essential</option><option value="pro">Pro</option></select></label></div><div class="onboarding-section"><div class="section-number">03</div><div><h4>Responsável pelo acesso</h4><p>O Supabase enviará o convite para este e-mail.</p></div></div><div class="form-grid"><label class="field"><span>Nome do responsável *</span><input name="ownerName" autocomplete="name" required></label><label class="field"><span>E-mail do responsável *</span><input name="ownerEmail" type="email" autocomplete="email" required></label></div><div class="modal-actions"><button type="button" class="btn btn-outline" data-tenant-close>Cancelar</button><button class="btn btn-dark" type="submit">Criar empresa e enviar convite</button></div></form></div></div>`);
  ...
  form.elements.name.addEventListener("input",()=>{if(!slugEdited)slug.value=slugify(form.elements.name.value);});
  form.onsubmit=async event=>{event.preventDefault();const button=event.currentTarget.querySelector("button[type=submit]");button.disabled=true;button.textContent="Criando empresa…";const values=Object.fromEntries(new FormData(event.currentTarget).entries());try{const result=await edge("tenant-onboarding",{action:"create",...values});await loadAdminData();document.querySelector("#tenant-modal")?.remove();render();showOnboardingSuccess(result);}catch(error){toast(error.message);button.disabled=false;button.textContent="Criar empresa e enviar convite";}};
}
...
function showOnboardingSuccess(result){
  ...
  document.body.insertAdjacentHTML("beforeend",`<div class="modal onboarding-modal" id="onboarding-success"><div class="modal-card success-card"><div class="success-mark">✓</div><div class="eyebrow">EMPRESA PREPARADA</div><h2>Convite enviado</h2><p>Enviamos o primeiro acesso para <b>${esc(result?.owner?.email||"")}</b>. Ao criar a senha, o responsável será levado ao login desta empresa.</p><div class="link-preview"><small>Link público</small><b>${esc(publicUrl)}</b></div><div class="modal-actions"><button class="btn btn-outline" data-success-close>Fechar</button><a class="btn btn-dark" target="_blank" rel="noopener" href="${publicUrl}">Abrir agenda</a></div></div></div>`);
...
function render() {
  ...
  app.innerHTML = PASSWORD_FLOW ? passwordPage() : location.hash === "#admin" ? (!authSession ? loginPage() : !adminLoaded ? loadingPage() : currentProfile?.role==="platform_admin" ? (PLATFORM_ENTRY?platformPage():tenantAdminGuardPage()) : currentSubscription?.status==="suspended" ? suspendedPage() : adminPage()) : CHRONA_HOME ? chronaHomePage() : publicPage();
  bind();
}
function bind() {
